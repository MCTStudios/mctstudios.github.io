(() => {
  ...
  el.htmlImportInput.onchange=async()=>{const f=el.htmlImportInput.files?.[0];if(f){const d=new DOMParser().parseFromString(await f.text(),'text/html');d.querySelectorAll('script,iframe,object,embed').forEach(n=>n.remove());el.composeBody.innerHTML=d.body?.innerHTML||'';el.htmlImportInput.value='';toast('HTML importato','Controlla il contenuto prima di inviare.')}};
