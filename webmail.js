(() => {
  ...
  const state = {
    ...
    loading: false,
    checkedIds: new Set(), composeAttachments: [], importedHtml: null
  };
  ...
  async function sendMessage(event) {
    ...
    try {
      await api('/send', {
        method: 'POST',
        body: JSON.stringify({account:state.account,to:recipients,cc:ccRecipients,bcc:bccRecipients,subject:el.composeSubject.value.trim(),text:state.importedHtml?'Messaggio HTML MCT Studios':el.composeBody.innerText.trim(),html:state.importedHtml||el.composeBody.innerHTML.trim(),attachments:await Promise.all(state.composeAttachments.map(async f=>({name:f.name,type:f.type||'application/octet-stream',size:f.size,data:String(await dataUrl(f)).split(',')[1]})))})
      });
      closeCompose(); el.composeForm.reset(); el.composeBody.innerHTML=''; el.composeBody.contentEditable='true'; el.composeBody.classList.remove('html-preview-mode'); state.importedHtml=null; state.composeAttachments=[]; renderFiles();
  ...
  try{
    ...
    state.importedHtml='<!doctype html>\n'+d.documentElement.outerHTML;

    el.composeBody.innerHTML='';
    el.composeBody.contentEditable='false';
    el.composeBody.classList.add('html-preview-mode');

    const preview=document.createElement('iframe');
    preview.className='html-email-preview';
    preview.setAttribute('title','Anteprima email HTML');
    preview.setAttribute('sandbox','allow-popups allow-popups-to-escape-sandbox');
    preview.srcdoc=state.importedHtml;
    el.composeBody.appendChild(preview);

    toast('HTML caricato','Anteprima fedele attiva: verrà inviato l’HTML originale.');
  }catch(err){
    state.importedHtml=null;
