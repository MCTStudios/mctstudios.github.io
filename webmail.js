(() => {
  ...
  const el = {
    ...
    composeTo: $('composeTo'), composeCc: $('composeCc'), composeBcc: $('composeBcc'), composeSubject: $('composeSubject'), composeBody: $('composeBody'), sendBtn: $('sendBtn'), composeToolbar:$('composeToolbar'), inlineImageInput:$('inlineImageInput'), attachmentInput:$('attachmentInput'), htmlImportInput:$('htmlImportInput'), composeFiles:$('composeFiles'),
    ...
    markAllRead: $('markAllReadBtn'), selectAll:$('selectAllMessages'), bulkActions:$('bulkActions'), selectedCounter:$('selectedCounter'), bulkRead:$('bulkReadBtn'), bulkUnread:$('bulkUnreadBtn'), bulkDelete:$('bulkDeleteBtn'), emptyFolder:$('emptyFolderBtn'), readerPanel: $('readerPanel'), readerPlaceholder: $('readerPlaceholder'),
  ...
  const state = {
    ...
    loading: false,
    checkedIds: new Set(), composeAttachments: []
  };
  ...
  function updateFolderChrome() {
    ...
    el.emptyFolder.hidden = !['trash','junk'].includes(state.folder); el.emptyFolder.textContent = state.folder === 'trash' ? 'Svuota cestino' : 'Svuota spam';
  }

  function updateBulk(){ const ids=state.filtered.map(m=>m.id), n=ids.filter(id=>state.checkedIds.has(id)).length; el.bulkActions.hidden=!state.checkedIds.size; el.selectedCounter.textContent=`${state.checkedIds.size} selezionati`; el.selectAll.checked=!!ids.length&&n===ids.length; el.selectAll.indeterminate=n>0&&n<ids.length; el.bulkDelete.textContent=state.folder==='trash'?'Elimina':'Cestino'; }
  async function bulkRead(read){ for(const id of [...state.checkedIds]){try{await api(`/messages/${encodeURIComponent(id)}/read`,{method:'POST',body:JSON.stringify({account:state.account,read})})}catch{}} state.checkedIds.clear(); await loadMessages({quiet:true}); }
  async function bulkDelete(){const ids=[...state.checkedIds];if(!ids.length)return;const perm=state.folder==='trash';if(!confirm(perm?`Eliminare definitivamente ${ids.length} messaggi?`:`Spostare ${ids.length} messaggi nel cestino?`))return;for(const id of ids){try{if(perm)await api(`/messages/${encodeURIComponent(id)}?account=${encodeURIComponent(state.account)}`,{method:'DELETE'});else await api(`/messages/${encodeURIComponent(id)}/move`,{method:'POST',body:JSON.stringify({account:state.account,folder:'trash'})})}catch{}}state.checkedIds.clear();closeReaderView();await loadMessages({quiet:true});}
  async function emptyFolder(){if(!state.messages.length)return;if(!confirm(`Svuotare completamente ${state.folder==='trash'?'il cestino':'lo spam'}?`))return;for(const m of [...state.messages]){try{await api(`/messages/${encodeURIComponent(m.id)}?account=${encodeURIComponent(state.account)}`,{method:'DELETE'})}catch{}}state.checkedIds.clear();closeReaderView();await loadMessages({quiet:true});}
  ...
  function renderFiles(){el.composeFiles.innerHTML='';el.composeFiles.hidden=!state.composeAttachments.length;state.composeAttachments.forEach((f,i)=>{const d=document.createElement('div');d.className='compose-file';d.innerHTML='<span></span><button type="button">✕</button>';d.querySelector('span').textContent=`${f.name} · ${Math.ceil(f.size/1024)} KB`;d.querySelector('button').onclick=()=>{state.composeAttachments.splice(i,1);renderFiles()};el.composeFiles.appendChild(d)})}
  ...
  function renderList() {
    ...
    state.filtered.forEach(msg => {
      ...
      const check=document.createElement('input');check.type='checkbox';check.className='message-check';check.checked=state.checkedIds.has(msg.id);check.onclick=e=>{e.stopPropagation();check.checked?state.checkedIds.add(msg.id):state.checkedIds.delete(msg.id);renderList()};row.classList.toggle('bulk-selected',check.checked);
      ...
      row.append(check,dot,content);
      row.addEventListener('click', () => openMessage(msg.id));
      frag.appendChild(row);
    });
    el.messageList.appendChild(frag); updateBulk();
  }
  ...
  async function sendMessage(event) {
    ...
    const ccRecipients = el.composeCc.value.split(',').map(v => v.trim()).filter(Boolean);
    const bccRecipients = el.composeBcc.value.split(',').map(v => v.trim()).filter(Boolean);
    ...
    try {
      await api('/send', {
        method: 'POST',
        body: JSON.stringify({account:state.account,to:recipients,cc:ccRecipients,bcc:bccRecipients,subject:el.composeSubject.value.trim(),text:el.composeBody.innerText.trim(),html:el.composeBody.innerHTML.trim(),attachments:await Promise.all(state.composeAttachments.map(async f=>({name:f.name,type:f.type||'application/octet-stream',size:f.size,data:String(await dataUrl(f)).split(',')[1]})))})
      });
      closeCompose(); el.composeForm.reset(); el.composeBody.innerHTML=''; state.composeAttachments=[]; renderFiles();
  ...
  document.querySelectorAll('.folder').forEach(btn => btn.addEventListener('click', async () => {
    state.folder = btn.dataset.folder; state.checkedIds.clear(); closeReaderView(); updateFolderChrome();
  ...
  el.composeToolbar.addEventListener('click',e=>{const b=e.target.closest('button[data-cmd]');if(b){e.preventDefault();el.composeBody.focus();document.execCommand(b.dataset.cmd,false,null)}});
  $('insertLinkBtn').onclick=()=>{const u=prompt('Inserisci il link');if(u){el.composeBody.focus();document.execCommand('createLink',false,u)}};
  el.inlineImageInput.onchange=async()=>{const f=el.inlineImageInput.files?.[0];if(f){el.composeBody.focus();document.execCommand('insertImage',false,await dataUrl(f));el.inlineImageInput.value=''}};
  el.attachmentInput.onchange=()=>{state.composeAttachments.push(...Array.from(el.attachmentInput.files||[]));el.attachmentInput.value='';renderFiles()};
  el.htmlImportInput.onchange=async()=>{
  const f=el.htmlImportInput.files?.[0];
  if(!f)return;
  try{
    const source=await f.text();
    const d=new DOMParser().parseFromString(source,'text/html');
    d.querySelectorAll('script,iframe,object,embed,meta,link').forEach(n=>n.remove());

    // Converte il CSS dell'HTML importato in stili inline.
    // In questo modo il design resta dentro l'email e non dipende dal <head>.
    const styleTexts=[...d.querySelectorAll('style')].map(s=>s.textContent||'');
    ...
    const importedRoot=document.createElement('div');
    importedRoot.innerHTML=d.body?.innerHTML||'';
    ...
    // Proprietà email-safe che copiamo dal CSS calcolato.
    const props=[
      ...
      'text-transform','white-space','width','max-width','min-width','height',
      'max-height','margin','margin-top','margin-right','margin-bottom','margin-left',
      'padding','padding-top','padding-right','padding-bottom','padding-left',
      'border','border-top','border-right','border-bottom','border-left',
      'border-radius','box-shadow','display','vertical-align','overflow-wrap'
    ];

    [...importedRoot.querySelectorAll('*')].forEach(node=>{
      ...
      node.setAttribute('style',generated+current);
    });

    // Conserva anche lo stile del BODY su un contenitore interno.
    const bodyStyle=getComputedStyle(d.body);
    const wrapper=document.createElement('div');
    let wrapperCss='';
    ['background','background-color','color','font-family','font-size','line-height','padding','text-align'].forEach(p=>{
      const v=bodyStyle.getPropertyValue(p);
      if(v && v!=='normal' && v!=='none' && v!=='auto' && v!=='0px')wrapperCss+=`${p}:${v};`;
    });
    wrapper.setAttribute('style',wrapperCss);
    wrapper.innerHTML=importedRoot.innerHTML;

    document.body.removeChild(host);
    el.composeBody.innerHTML=wrapper.outerHTML;
    toast('HTML importato','Design e CSS convertiti per l’email.');
  }catch(err){
    toast('Importazione HTML non riuscita',err.message,'error');
  }finally{
    el.htmlImportInput.value='';
  }
};
  el.selectAll.onchange=()=>{state.filtered.forEach(m=>el.selectAll.checked?state.checkedIds.add(m.id):state.checkedIds.delete(m.id));renderList()};
  el.bulkRead.onclick=()=>bulkRead(true);el.bulkUnread.onclick=()=>bulkRead(false);el.bulkDelete.onclick=bulkDelete;el.emptyFolder.onclick=emptyFolder;
